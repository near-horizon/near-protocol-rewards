...
export class GitHubRewardsCalculator {
  ...
  private calculateScore(metrics: GitHubMetrics): Score {
    const commitScore = Math.min(metrics.commits.count / this.thresholds.commits, 1) * this.weights.commits * 100;
    const prScore = Math.min(metrics.pullRequests.merged / this.thresholds.pullRequests, 1) * this.weights.pullRequests * 100;
    const reviewScore = Math.min(metrics.reviews.count / this.thresholds.reviews, 1) * this.weights.reviews * 100;
    const issueScore = Math.min(metrics.issues.closed / this.thresholds.issues, 1) * this.weights.issues * 100;
  ...
  private calculateAchievements(metrics: GitHubMetrics): RewardCalculation['achievements'] {
